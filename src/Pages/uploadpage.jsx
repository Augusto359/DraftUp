...
export default function Upload() {
  ...
  return (
    ...
    <div className="w-full min-h-screen bg-cover bg-center absolute"
        ...
        <div id="header">
          <header className="flex justify-between items-center p-4">
            ...
            <nav className="flex gap-4">
                <Link to="/" className='text-blue-600 underline'>Início</Link>
      ...
      <label className='bg-white border-dashed hover:shadow-xl hover:border-blue-400 text-black px-2 py-2 border border-black rounded-md'>
        <div className='gap-4 p-6 flex justify-center flex-col items-center'>
          ...
          <p>Selecione um:</p>
          <span className='bg-slate-200 border rounded-xl hover:border-blue-400 p-4'>Escolher um arquvivo</span>
          ...
          <button disabled={!arquivo} className='bg-white hover:shadow-xl hover:border-blue-400 text-black px-4 py-4 border rounded-md mb-4 m-6 disabled:bg-slate-400 disabled:shadow-none disabled:cursor-not-allowed'>Envie sua planta</button>
            </div>
          ...
          {mostrarcard && (
            <div className='fixed inset-0 bg-black/50 text-center flex justify-center items-center'>
              <div className='bg-white rounded-lg py-5 px-5 p-5 flex flex-col gap-4 break-inside-auto'>
                ...
                <button className='bg-slate-200 border rounded-lg border-slate-400 hover:border-blue-400 hover:shadow-xl px-4 py-4 '>Sim</button> <br />
                <button onClick={() => setMostrarcard(false)} className='bg-slate-200 border rounded-lg border-slate-400 hover:border-blue-400 hover:shadow-xl px-4 py-4'>Cancelar</button>
              </div>
            </div>
          )}

          <button onClick={() => setMostrarcard(true)} className='bg-white justify-center col-span-2 w-fit p-4 mx-auto border border-slate-400 rounded-xl hover:border-blue-400 hover:shadow-lg transition-shadow'>Não tem uma planta?</button>
            </section>
      </div>
